...
#endif
=======
>>>>>>> 201cc11afa0a1950e1f632390b2ac6c937a0d8f0
#ifndef QK_K
#ifdef GGML_QKK_64
#define QK_K 64
#else
...
>>>>>>> 201cc11afa0a1950e1f632390b2ac6c937a0d8f0
        ...
        else
        {
            ...
            case 26:
                ...
                break;
            case 32:
                model.type = hparams.n_head == hparams.n_head_kv ? e_model::MODEL_7B : e_model::MODEL_8B;
                break; // LLaMa 8B v3 uses GQA
            case 40:
...
struct llama_logit_info
{
    ...
    llama_logit_info(llama_context *ctx)
        : logits(llama_get_logits(ctx)), n_vocab(llama_n_vocab(llama_get_model(ctx))), max_l(*std::max_element(logits, logits + n_vocab)), normalizer(1.0f / std::accumulate(logits, logits + n_vocab, 0.0f, sum_exp{max_l}))
    {
    }
...
struct llama_beam_search_data
{
    ...
    void loop(const llama_beam_search_callback_fn_t callback, void *const callback_data)
    {
        ...
        const auto not_eob = [](const llama_beam &beam)
        { return !beam.eob; };
             ...
             ++i)
        {
            ...
            std::for_each(next_beams.begin(), next_beams.end(), [](llama_beam &beam)
                          { beam.p = 0.0f; });
    ...
    static void renormalize_beam_probabilities(std::vector<llama_beam> &beams)
    {
        ...
        std::for_each(beams.begin(), beams.end(), [=](llama_beam &beam)
                      { beam.p *= inv_sum; });
    }
                       ...
                       size_t n_beams, int n_past, int n_predict)
{
    ...
    ctx->t_sample_us += ggml_time_us() - t_start_sample_us;
    printf("Beam search time: %ld s\n", ctx->t_sample_us / 1000000);
    ctx->n_sample++;
}
...
>>>>>>> 201cc11afa0a1950e1f632390b2ac6c937a0d8f0
    ...
    cparams.n_yarn_orig_ctx = params.yarn_orig_ctx != 0 ? params.yarn_orig_ctx : hparams.n_yarn_orig_ctx != 0 ? hparams.n_yarn_orig_ctx
                                                                                                              : hparams.n_ctx_train;
...
size_t llama_state_get_size(const struct llama_context *ctx)
{
    ...
    const size_t s_total = (+s_rng_size + s_rng + s_n_outputs + s_output_pos + s_logits_size + s_logits + s_embedding_size + s_embedding + s_kv_buf_size + s_kv_head + s_kv_size + s_kv_used + s_v_trans + s_kv + s_kv_cells);
...
size_t llama_state_set_data(struct llama_context *ctx, const uint8_t *src)
{
    ...
    // set rng
    {
        ...
        std::string rng_str((const char *)inp, rng_size);
        inp += rng_size;
    ...
    // set output ids
    {
        ...
        memcpy(&n_outputs, inp, sizeof(n_outputs));
        inp += sizeof(n_outputs);
    ...
    // set embeddings
    {
        ...
        memcpy(&embeddings_size, inp, sizeof(embeddings_size));
        inp += sizeof(embeddings_size);
    ...
    // set kv cache
    {
        ...
        inp += sizeof(kv_size);
        memcpy(&kv_used, inp, sizeof(kv_used));
        inp += sizeof(kv_used);
        memcpy(&v_trans, inp, sizeof(v_trans));
        inp += sizeof(v_trans);
...
size_t llama_state_seq_get_size(struct llama_context *ctx, llama_seq_id seq_id)
{
    ...
    const size_t s_total = (size_t_size_size +
                            s_cell_count_size +
                            s_layer_count_size +
                            n_embd_v_gqa_size +
                            s_cell_data_size);
...
{
    return token != -1 && (token == llama_token_eos(model) ||
