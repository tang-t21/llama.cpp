...
int main(int argc, char **argv)
{
    ...
    if (argc == 1 || argv[1][0] == '-')
    {
        printf("usage: %s MODEL_PATH [N_KV_MAX] [N_BATCH] [N_UBATCH] [FATTN] [IS_PP_SHARED] [NGL] <PP> <TG> <PL> [prompt0] [prompt1]...\n", argv[0]);
    ...
    // std::vector<std::vector<llama_token>> tokens_lists(prompts.size(), std::vector<llama_token>());
    // for (int i = 0; i < prompts.size(); i++)
    // {
    //     process_escapes(prompts[i]);
    //     tokens_lists[i] = llama_tokenize(model, prompts[i], true);
    // }
    ...
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int> distrib(0, llama_n_vocab(model) - 1);
        ...
        for (int i_tg = 0; i_tg < (int)n_tg.size(); ++i_tg)
        {
            for (int i_pl = 0; i_pl < (int)n_pl.size(); ++i_pl)
            {
                ...
                for (int i = 0; i < pp; ++i)
                {
                    ...
                    {
                        llama_batch_add(batch, distrib(gen), i, {j}, false);
                    }
                }
                ...
                for (int i = 0; i < tg; ++i)
                {
                    ...
                    for (int j = 0; j < pl; ++j)
                    {
                        llama_batch_add(batch, distrib(gen), pp + i, {j}, true);
                    }
